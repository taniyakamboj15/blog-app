...
import { Lock, Mail, User, Loader, Eye, EyeOff, ArrowRight } from 'lucide-react';
        ...
        <div className="flex justify-center items-center min-h-[calc(100vh-100px)] animate-in fade-in zoom-in-95 duration-500 py-12">
            <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-8 md:p-10 border border-gray-100 dark:border-gray-800 relative overflow-hidden">
                ...
                <div className="text-center mb-10 relative">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">{t('create_account_desc')}</p>
                </div>
                ...
                <form onSubmit={handleSignup} className="space-y-5 relative">
                    <div className="space-y-2">
                        <label className="text-sm font-semibold text-gray-700 dark:text-gray-300 ml-1">{t('username_label')}</label>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-semibold text-gray-700 dark:text-gray-300 ml-1">{t('role_label')}</label>
                        <select
                            value={role}
                            onChange={(e) => setRole(e.target.value)}
                            className="w-full px-4 py-3.5 rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all text-gray-900 dark:text-white"
                        >
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full flex items-center justify-center gap-2 py-4 px-4 bg-gradient-to-r from-indigo-600 to-pink-600 hover:from-indigo-700 hover:to-pink-700 text-white font-bold rounded-2xl shadow-xl shadow-indigo-500/30 transform transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none mt-4"
                    >
                ...
                <p className="mt-8 text-center text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 py-3 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <Link to="/login" className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-pink-600 hover:underline decoration-indigo-500/30 underline-offset-4">
                        {t('login')}
                    </Link>
                </p>
            </div>
